...
$s\nearrow1$ limit; and the tail bookkeeping below. With these ingredients
the \textbf{main growth estimate is proved modulo one Liouville statement}
(contradiction $+$ a $\theta_k$ blow-up run at a generic sub-sharp
exponent, the absorption killed by homogeneity, the limit killed by the
Liouville). The only open inputs are that Liouville in the nonlocal
setting and a nondegeneracy property; see \S4.
...
\item \textbf{The nonlocal Liouville --- which route?} Your \S6 (and
  Correa--dos Prazeres) prove it by rescaling $v$ and applying the sharp
  $C^{1,\al_0}$ estimate to $v_l=v(l\cdot)/l^{1+\beta}$. Nonlocally this
  seems blocked: $v_l$ keeps the growth $1+\beta=\al$, and
  $\mathrm{Tail}(v_l)<\infty$ would need $\al(p-1)<sp$, whereas
  $\al(p-1)-sp=\frac{\gamma ps}{p-1-\gamma}>0$ for every $\gamma>0$
  (truncating leaves a datum with the same divergent exponent). So I expect
  one must pass to \emph{increments} $w^h$ (which lower the growth to
  $\beta<1$ and have finite tail under the tail condition), giving the cap
  $\al<2$ rather than $\al<1+\al_0$ --- with $\al_0$ then needed only for
  compactness, not quantitatively. Two questions: (a) is the direct route
  really unavailable nonlocally, or is there a way to apply the estimate
  past the divergent tail? (b) for the increment route, the linearized
  kernel $K_v\simeq|v(x)-v(y)|^{p-2}|x-y|^{-n-sp}$ degenerates where $v$ is
  flat; is there a clean way to get \emph{nondegeneracy} of the blow-up
  limit $v_0$ (gradient not identically zero on open sets) so the kernel is
  uniformly elliptic? These two are now the only open inputs to the main
  theorem.
