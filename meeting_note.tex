...
$s\nearrow1$ limit; and the tail bookkeeping below.
...
\section{A proposed argument (in outline; unchecked)}
...
\end{enumerate}
No lower bound on $\theta_k$ is needed. If correct, the same
far-field-as-datum mechanism should carry to $p\neq2$, where the
increments solve the \emph{linear} equation $L_{K_{v_k}}w=g$ with the
kernel $K_v\simeq|v(x)-v(y)|^{p-2}|x-y|^{-n-sp}$ of GJS \S7; the
oscillation computation there produces the additional condition
$\gamma<\frac{p-1}{1+sp}$ (which coincides with $\frac{1}{1+2s}$ at
$p=2$ and is stricter for $p>2$).

\section{What I would like to discuss}
(1) Whether \S2--\S3 above are correct or whether (4.12) has a
standard justification I missed. (2) The plan to prove the local
two-phase $p$-dead-core theorem directly (the $s\nearrow1$ route is
blocked by the non-uniformity of the GJS constants); it does not seem
to appear in the literature and may stand alone. (3) Whether to include
the $f\in L^\infty$ extension of GJS for $p<\frac{1}{1-s}$ as a section
of our paper.
