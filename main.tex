...
restated 2026-06-10 after \cite{CP}]\label{conj:liouville}
...
\]
for some $0<\beta<\al_0$, where $\al_0$ is the exponent of the interior
$C^{1,\al_0}$ estimate \cite{GJS}. Then $v\equiv0$.
\end{conjecture}
\begin{remark}
The earlier formulation (seminorm growth, conclusion ``$v$ is affine'')
required subtracting the affine part, which destroys the equation for
$p\neq2$. The vanishing-jet form avoids this entirely and suffices for
the application, since the blow-up limit inherits
$v_0(0)=|Dv_0(0)|=0$ from the free boundary point. The jet-to-growth
Lemma~\ref{lem:jet} below converts the seminorm information produced by
the $\theta_k$ device into the growth hypothesis above.
\end{remark}
...
\]
a nonempty range because $\al>1$ on the admissible set and, by the ceiling
constraint (C3) $\al<1+\al_0$, i.e.\ $\al-1<\al_0$. Lemma~\ref{lem:ceiling}
gives $u\in C^{1+\al'}_{loc}$ with a uniform bound. \emph{This $\al'$ is
generic and sub-sharp; the sharp $\al_0$ enters only through the Liouville
cap below.} We may take $x_0=0$, $\|u\|_{L^\infty(\R^n)}=1$, and must show
$|u(x)|\le C|x|^{\al}$.
...
$\al$ satisfies $\al<1+\al_0$ (C3), so the hypothesis of
Conjecture~\ref{conj:liouville} holds and $v_0\equiv0$, contradicting
$[v_0]_{C^{1+\al'}(B_1)}\ge\frac12$.
\end{proof}
\NOTE{\textbf{On the exponent bookkeeping (resolved 2026-06-15 against
\cite{CP} \S6 line by line; supersedes the earlier ``crux'' flag).} The
device runs at a \emph{generic, sub-sharp} exponent
$\al'<\min\{\al_0,\al-1\}$ --- \emph{not} at level $1+\al_0$. The three
exponents sit as
\[
  \underbrace{1+\al'}_{\text{generic working level}}
  \;<\;\al\;<\;\underbrace{1+\al_0}_{\text{sharp ceiling / Liouville cap}},
\]
exactly CP's arrangement (their $\theta_k$ exponent
$1+\al'-\al<0$ with $\al'<\al_p$, while Lemma~5 caps at $\al<1+\al_p$).
There is \emph{no} contradiction: the target lies above the level the
device measures and below the sharp level the Liouville uses. The earlier
worry came from misidentifying the working level with $1+\al_0$. So
Conjecture~\ref{conj:liouville} (cap $\al<1+\al_0$) is the correct
statement, and the synthesis finding (iii) stands: \textbf{the genuine
binding constraint is $\al<1+\al_0$ with $\al_0$ the \emph{sharp} \cite{GJS}
exponent}, i.e.\ a quantitative lower bound on $\al_0$ is what the result
needs. The increment/translation-invariance machinery of
Remark~\ref{rem:cprime-check} is \emph{not} required here --- it belongs to
the separate PTU-(4.12) repair. The one genuinely open input is
Conjecture~\ref{conj:liouville} itself in the nonlocal setting (its
\cite{CP}-style one-shot rescaling needs the sharp estimate applied to
$v_l$, whose tail diverges --- closed by truncation $+$ the $f\in L^\infty$
extension of \cite{GJS}; see the note after Conjecture~\ref{conj:liouville}).}
