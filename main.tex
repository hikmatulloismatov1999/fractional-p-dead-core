...
\end{proposition}
\begin{proof}
\TODO{Write out the change of variables $y\mapsto ry$ in the singular
integral: the difference contributes $r^{-\al(p-1)}$ through $\Jp{\cdot}$,
the kernel and the volume element contribute $r^{sp}$. Then match against
$(u_r)_+^{\gamma}-(u_r)_-^{\gamma} = r^{-\al\gamma}\big(u_+^{\gamma}-u_-^{\gamma}\big)(rx)$.
Check consistency: $p=2$ gives $\al=\frac{2s}{1-\gamma}$ (PTU);
$s\nearrow1$ gives $\al=\frac{p}{p-1-\gamma}$ (da Silva--Salort).}
\end{proof}
\begin{remark}
...
\begin{itemize}
  ...
  \item \textbf{$C^{1,\al_0}$ paper} ([28] in arXiv:2509.15988):
        shopping list --- exact statement, constants, uniformity in $s$.
