...
- [x] ✅ **EXPONENT BOOKKEEPING SETTLED 2026-06-15 (after TWO corrections; main.tex NOTE after `thm:main` + `rem:routeforced`).** (a) The "crux" panic was a false alarm — the device runs at generic sub-sharp $\alpha'<\min\{\alpha_0,\alpha-1\}$, NOT at $1+\alpha_0$; $\alpha_0$ only for compactness (any $\alpha_0>0$). (b) The opposite over-correction ("binding = sharp $\alpha<1+\alpha_0$, CP-style") is ALSO wrong nonlocally: **CP's direct rescaling is BLOCKED by the tail** — $\alpha(p-1)-sp=\frac{\gamma ps}{p-1-\gamma}>0$ for ALL $\gamma>0$ (verified), truncation diverges too. **The Liouville MUST go via increments (route b), cap $\beta<1$ i.e. $\boxed{\alpha<2}$ (C2/`lem:beta`).** ⇒ **Binding constraints = C1 (tail) + C2 ($\alpha<2$), both EXPLICIT; NO quantitative $\alpha_0$ needed. Finding (iii) SUPERSEDED.** New price = **nondegeneracy of $v_0$** + a linear nonlocal Liouville. `conj:liouville` restated with cap $\beta<1$ + nondegeneracy.
